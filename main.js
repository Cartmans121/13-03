...
(function () {
    ...
    function loadMap() {
        ...
        ymaps.ready(function () {
            ...
            const address = 'ул. Кубанская набережная, 37/10, Краснодар, Краснодарский край, 350063';
            const phoneNumber = '8 (914) 858-56-34';
            const contactName = 'Вадим Сергеевич';

            const socialLinks = [

                {
                    url: '#',
                    icon: './img/map/whatsapp.png',
                    alt: 'WhatsApp'
                },
                {
                    url: '#',
            ...
            for (const link of socialLinks) {
                socialLinksHTML += `<li><a target="_blank" href="${link.url} class="social-link"><img src="${link.icon}" wodth="23" alt=${link.alt} /></a></li>`
            }
            ...
            <div class="marker-content">
                <div class="marker-container">
                    <div class="marker-address mb-4">
                        <p class="text text-marker m-0 mb-3 p-0">Наш адрес:</p>
                        <address class="m-0 text">${address}</address>
                </div>
                <div>
                    <p class="text text-marker m-0 mb-2 p-0">Телефон для связи:</p>
                    <a href="tel:${phoneNumber}" class="text marker-link link-offset-3-hover">${phoneNumber}</a>
                    <h6 class="marker-name text m-0 mt-2">${contactName}</h6>
                </div>
                <div>
                    <p class="text text-marker p-0 m-0 mb-2">Мессенджеры и соц. сети:</p>
                    <ul class="p-0 m-0 d-flex gap-4">${socialLinksHTML}</ul>
                </div>
            </div>
        `;
